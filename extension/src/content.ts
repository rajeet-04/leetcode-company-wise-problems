...
import { createLocationWatcher } from "./problem-detector";
...
async function renderForSlug(slug: string | null) {
  ...
  score.textContent = `${priority.score}`;
  const title = document.createElement("div");
  title.className = "lp-title";
  title.textContent = problem.title;
  const meta = document.createElement("div");
  meta.className = "lp-meta";
  meta.textContent = `${problem.difficulty || "—"} · ${companies.length} ${companies.length === 1 ? "company" : "companies"}`;
  const button = document.createElement("button");
  button.type = "button";
  button.textContent = "Open panel";
  button.addEventListener("click", () => void chrome.runtime.sendMessage({ type: "panel:open" }));
