...
export type ExtensionRequest = LookupRequest | SetCurrentRequest | GetCurrentRequest | OpenPanelRequest | ConfigurePanelRequest | HistoryStatusRequest | SyncExchangeRequest | SubmissionRequest | HistoryImportRequest;
...
export function isExtensionRequest(value: unknown): value is ExtensionRequest {
  ...
  return ["problem:lookup", "state:set-current", "state:get-current", "panel:open", "panel:configure", "sync:exchange", "progress:submission", "progress:history-import", "progress:history-status"].includes(String(type));
}
