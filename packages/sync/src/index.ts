...
export type MutationEnvelope<TType extends string, TPayload> = {
  protocolVersion: typeof SYNC_PROTOCOL_VERSION; schemaVersion: typeof SYNC_SCHEMA_VERSION;
  mutationId: string; installationId: string; source: SyncSource; type: TType; occurredAt: string; payload: TPayload;
};
...
function validProgress(value: unknown): value is ProblemProgress {
  ...
  return validSlug(value.slug) && ["unseen", "attempted", "solved", "revision_due", "mastered"].includes(String(value.status)) && typeof value.attempts === "number" && Number.isFinite(value.attempts) && value.attempts >= 0 && typeof value.revisitCount === "number" && Number.isFinite(value.revisitCount) && value.revisitCount >= 0;
}
...
export function compareMutations(a: ProgressMutation, b: ProgressMutation): number { return a.occurredAt.localeCompare(b.occurredAt) || a.mutationId.localeCompare(b.mutationId); }
export function mergeMutations(current: readonly ProgressMutation[], incoming: readonly ProgressMutation[]): ProgressMutation[] {
  ...
  for (const mutation of [...current, ...incoming]) if (validateMutation(mutation) && !byId.has(mutation.mutationId)) byId.set(mutation.mutationId, mutation);
...
export function missingMutations(mutations: readonly ProgressMutation[], knownMutationIds: readonly string[]): ProgressMutation[] { const known = new Set(knownMutationIds); return mutations.filter((mutation) => !known.has(mutation.mutationId)).sort(compareMutations); }
export function bootstrapProgressMutations(progress: readonly ProblemProgress[], installationId: string, source: SyncSource): ProgressMutation[] {
  return progress.map((record) => { const occurredAt = record.lastAttemptAt ?? record.solvedAt ?? record.masteredAt ?? record.firstSeenAt ?? new Date(0).toISOString(); return { protocolVersion: 1, schemaVersion: 1, mutationId: `${source}:bootstrap:${record.slug}:${occurredAt}`, installationId, source, type: "PROBLEM_STATE_SET", occurredAt, payload: { progress: { ...record } } }; });
}
