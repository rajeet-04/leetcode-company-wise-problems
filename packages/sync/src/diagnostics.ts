...
export function deriveSyncDiagnostics(input: SyncDiagnosticInput, now: string): SyncDiagnostics {
  ...
  if (input.lastError === "bridge-timeout" && !Number.isFinite(successMs)) state = "unavailable";
  else if (input.lastError && (!Number.isFinite(successMs) || !Number.isFinite(attemptMs) || attemptMs >= successMs)) state = "error";
