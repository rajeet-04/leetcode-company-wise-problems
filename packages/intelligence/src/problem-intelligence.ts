...
import { companyOverlap, recencyScore, scoreProblemPriority, sourceFrequencyScore, trendScore, type ScoreResult } from "./core";
...
export function buildProblemIntelligence(problem: CatalogProblem, input: ProblemIntelligenceInput = {}): ProblemIntelligence {
  ...
  const priority = scoreProblemPriority(problem, { targetCompanies, solved, attempted, revisionDue: progress?.status === "revision_due", planRelevant: input.planRelevant, weakTopicMatches: input.weakTopicMatches });
  return {
    slug: problem.slug,
    ...
    revisionDueAt: progress?.revisionDueAt ?? null,
    priority,
  };
}
